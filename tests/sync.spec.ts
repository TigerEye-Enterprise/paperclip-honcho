...
describe("honcho sync", () => {
  ...
  it("caps oversized comment ingests to a bounded message length", async () => {
    const { requests } = installFetchMock();
    const fixtures = buildDefaultFixtures();
    const harness = createHonchoHarness({
      seed: {
        issueComments: [
          ...fixtures.issueComments,
          {
            ...fixtures.issueComments[1],
            id: "c_3",
            body: `Intro\n\n${"A".repeat(DEFAULT_MAX_INGEST_MESSAGE_CHARS + 500)}`,
            createdAt: new Date("2026-03-15T12:03:00.000Z"),
            updatedAt: new Date("2026-03-15T12:03:00.000Z"),
          },
        ],
    ...
    expect(messageRequests).toHaveLength(1);
    const messages = (messageRequests[0]?.body?.messages ?? []) as Array<Record<string, unknown>>;
    const oversizedComment = messages.find((message) => (message.metadata as Record<string, unknown>)?.commentId === "c_3");
    expect(typeof oversizedComment?.content).toBe("string");
    expect(String(oversizedComment?.content).length).toBeLessThanOrEqual(DEFAULT_MAX_INGEST_MESSAGE_CHARS);
    expect(String(oversizedComment?.content).endsWith("…")).toBe(true);
  });
  ...
  it("isolates document sync failures so the event bus does not reject the event", async () => {
    ...
    await expect(harness.emit("issue.updated", { key: "design" }, {
      entityId: "iss_1",
      entityType: "issue",
      companyId: "co_1",
    ...
    expect(harness.logs).toContainEqual(expect.objectContaining({
      level: "warn",
      message: "Honcho sync on issue.updated failed",
    }));
  });
